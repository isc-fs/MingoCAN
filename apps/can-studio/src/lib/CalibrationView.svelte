<!--
    Pedal calibration (#534). Lives under Program, not Observe: unlike
    every pit-diag view this one WRITES to the car, and the values it
    writes gate the EV.2.3 torque cut and the driverless R2D.
...
<script lang="ts">
    ...
    let pedals = $state<PedalReads | null>(null);

    // Preconditions come from 0x700 / 0x707, which the same stream carries.
    let fsmState = $state<string | null>(null);
    let tsActive = $state<boolean | null>(null);
    let motorRpm = $state<number | null>(null);
    let torquePct = $state<number | null>(null);
    ...
    function request() {
        return {
            ...
            profile: 'ecu' as const,
        };
    }

    onMount(async () => {
        unlistenStatus = await onPitDiagStatus((s: PitDiagStatus) => {
            if (s.kind === 'armed') armed = true;
            if (s.kind === 'stopped') armed = false;
            if (s.kind === 'error') {
                armed = false;
                error = s.message;
            }
        ...
        unlistenFrame = await onPitDiagFrame((event) => {
            if (event.kind === 'ecuPedals') {
                ...
                window_ = [
                    ...
                    { t: now, apps1: event.apps1Raw, apps2: event.apps2Raw, brake: event.brakeRaw },
                ];
                ...
                motorRpm = event.motorRpmMech;
            } else if (event.kind === 'calStatus') {
                status = {
    ...
    const arm = () => run(async () => { await pitDiagEnable(request()); });
    const disarm = () => run(async () => { await pitDiagDisable(request()); armed = false; });
...
<section class="view">
    ...
    {#if !armed}
        <div class="card">
            ...
            <p class="muted small">
                The wizard reads live pedal values from the ECU telemetry stream — without it
                there is nothing to capture. Arming also opens the path calibration commands
                travel on.
            </p>
            <button class="btn" onclick={arm} disabled={busy || settings.adapter.interface === null}>
                Arm ECU telemetry
            </button>
            {#if settings.adapter.interface === null}
                <p class="muted small">
                    No adapter selected — <button class="btn-link" onclick={() => navigateTo('adapters')}>choose one first</button>.
                </p>
            {/if}
        </div>
    {:else}
        ...
        <div class="badge-row">
            <button class="btn" onclick={disarm} disabled={busy}>Disarm telemetry</button>
        </div>
    {/if}
</section>
